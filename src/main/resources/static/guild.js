...
var channelList = document.querySelector("#channels ul")
var serverSettings = document.querySelector("#serverSettings")
var guildNameElement = document.querySelector("#guildName")

var stompClient = null;
var username = document.querySelector("html").getAttribute("data-user");
var guildId = document.querySelector("html").getAttribute("data-guildId");
var guildName = guildNameElement.innerHTML
var channelId = document.querySelector("html").getAttribute("data-channelId");

var fetchedMessages = Array()

const maxChannelNameLenght = 32
...
function onConnected() {
    ...
    stompClient.subscribe('/topic/guild/'+guildId+"/"+channelId, onMessageReceived);
    stompClient.subscribe("/topic/guild/"+guildId, guildSub)
...
function guildSub(payload) {
    var guild = JSON.parse(payload.body)
    var type = guild.type
    var action = guild.actionType

    if (type == "CHANNEL") {
        if (action == "CREATE") {
            onChannelCreate(guild)
        }
    } else if (type == "GUILD") {
        if (action == "RENAME") {
            onGuildRename(guild)
...
async function fetchMessages(numberOfMessages) {
    try {
        const response = await fetch(`/api/guild/channel/${channelId}/getMessages/${numberOfMessages}`);
...
document.addEventListener("click", function(event) {
    ...
    var menu = document.querySelector("#serverSettingsMenu")
    var btn = document.querySelector("#serverSettings")
    if (menu && btn && !btn.contains(event.target)) {
        removeServerSettingsMenu()
    }

    var CCF = document.querySelector(".channelCreatorForm")
    if (CCF && !(CCF.contains(event.target) || (menu && menu.contains(event.target)))) {
        removeChannelCreator()
    }

    var GRF = document.querySelector(".guildRenamerForm")
    if (GRF && !(GRF.contains(event.target) || (menu && menu.contains(event.target)))) {
        removeGuildRenamer()
    }

    var GDF = document.querySelector(".guildDeletorForm")
    if (GDF && !(GDF.contains(event.target) || (menu && menu.contains(event.target)))) {
        removeGuildDeletor()
    }
})
...
function removeServerSettingsMenu() {
    document.querySelectorAll("#serverSettingsMenu").forEach(ele => ele.remove())
}

function serverSettingsMenu(event) {
    removeServerSettingsMenu()

    var menu = buildServerSettingsMenu()
    document.body.appendChild(menu)
    menu.style.top = event.y + "px"
    menu.style.left = event.x + "px"
}

function buildServerSettingsMenu() {
    var menuElement = document.createElement("div")
        menuElement.id = "serverSettingsMenu"

    var groupElement = document.createElement("div")
        groupElement.appendChild(buildSetting("Rename guild", renameGuild, "edit"))
        groupElement.appendChild(buildSetting("Create Channel", createChannel, "add"))
        var delElement = buildSetting("Delete guild", delGuild, "del")
            delElement.classList.add("text-danger")
        groupElement.appendChild(delElement)
    menuElement.appendChild(groupElement)
...
function renameGuild() {
    removeGuildRenamer()
    document.body.appendChild(buildGuildRenamer())
    document.querySelector(".GRF-body input").focus()
}

function createChannel() {
    removeChannelCreator()
    document.body.appendChild(buildChannelCreator())
    document.querySelector(".CCF-body input").focus()
}

function delGuild() {
    removeGuildDeletor()
    document.body.appendChild(buildGuildDeletor())
}

function sendChannelCreate(event) {
    event.preventDefault()

    removeChannelCreator()

    var name = event.target.name.value
    if (name.length == 0) { warnHandler("Cannot create channel: Channel name cannot be empty"); return }
    if (name.length > maxChannelNameLenght) { warnHandler(`Cannot create channel: Length of channel name exceeds ${maxChannelNameLenght} characters`); return }
    ...
    if(stompClient) {
        ...
        stompClient.send("/app/chat.createChannel", {}, JSON.stringify(channel));
    }
}

function onChannelCreate(channel) {
    channelList.appendChild(buildChannel(channel))
    document.body.style.cursor = "auto"
}

function sendGuildDel(event) {
    event.preventDefault()
    ...
    if(stompClient) {
        ...
        stompClient.send("/app/chat.deleteGuild", {}, JSON.stringify(guild));
    }
}

function onDelGuild() {
    window.location.href = "/guild/@me"
}

function sendGuildRename(event) {
    event.preventDefault()

    removeGuildRenamer()

    var name = event.target.name.value
    if (name.length == 0) { warnHandler("Cannot rename guild: Guild name cannot be empty"); return }
    if (name.length > maxGuildNameLength) { warnHandler(`Cannot rename guild: Length of guild name exceeds ${maxGuildNameLength} characters`); return }
    ...
    if(stompClient) {
        ...
        stompClient.send("/app/chat.renameGuild", {}, JSON.stringify(guild));
    }
}

function onGuildRename(guild) {
    var name = guild.name
    guildNameElement.innerHTML = name
    guildName = name
}
...
function buildChannel(channel) {
    var element = document.createElement("li")
        ...
        var link = document.createElement("a")
            link.classList.add("fs-5", "m-0")
            link.href = `/guild/${channel.guildId}/${channel.id}`
            var linkText = document.createTextNode(channel.name)
            link.appendChild(linkText)
        element.appendChild(link)
    return element
}

function buildChannelCreator() {
    var layer = document.createElement("div")
        layer.classList.add("channelCreatorLayer", "layer")

        var form = document.createElement("form")
            form.classList.add("channelCreatorForm", "layer-form", "bg-body")

            var header = document.createElement("div")
                header.classList.add("CCF-header", "LF-header")
                var div = document.createElement("div")
                    var divText = document.createTextNode("Create Channel")
                    div.appendChild(divText)
                header.appendChild(div)
                var closeBtn = document.createElement("button")
                    closeBtn.classList.add("btn", "opacity-control")
                    closeBtn.type = "button"
                    closeBtn.addEventListener("click", removeChannelCreator, true)
                    closeBtn.appendChild(getSVG("close"))
                header.appendChild(closeBtn)
            form.appendChild(header)

            var body = document.createElement("div")
                body.classList.add("CCF-body", "LF-body")
                var label = document.createElement("label")
                    var labelText = document.createTextNode("Channel Name")
                ...
                var input = document.createElement("input")
                    ...
                    input.maxLength = maxChannelNameLenght
                body.appendChild(input)
            form.appendChild(body)

            var footer = document.createElement("div")
                footer.classList.add("CCF-footer", "LF-footer")
                var cancelBtn = document.createElement("button")
                    cancelBtn.classList.add("btn", "opacity-control")
                    cancelBtn.type = "button"
                    cancelBtn.addEventListener("click", removeChannelCreator, true)
                ...
                var submitBtn = document.createElement("button")
                    ...
                    var submitBtnText = document.createTextNode("Create Channel")
        ...
        form.addEventListener("submit", sendChannelCreate, true)
        input.addEventListener("input", function() { if (input.value.trim() == "") { submitBtn.disabled = true } else { submitBtn.disabled = false } }, true)

    return layer
}

function buildGuildDeletor() {
    var layer = document.createElement("div")
        layer.classList.add("guildDeletorLayer", "layer")

        var form = document.createElement("form")
            form.classList.add("guildDeletorForm", "layer-form", "bg-body")

            var header = document.createElement("div")
                header.classList.add("GDF-header", "LF-header")
                var div = document.createElement("div")
                    var divText = document.createTextNode("Delete " + guildName)
                    div.appendChild(divText)
                header.appendChild(div)
                var closeBtn = document.createElement("button")
                    closeBtn.classList.add("btn", "opacity-control")
                    closeBtn.type = "button"
                    closeBtn.addEventListener("click", removeGuildDeletor, true)
                    closeBtn.appendChild(getSVG("close"))
                header.appendChild(closeBtn)
            form.appendChild(header)

            var footer = document.createElement("div")
                footer.classList.add("GDF-footer", "LF-footer")
                var cancelBtn = document.createElement("button")
                    cancelBtn.classList.add("btn", "btn-secondary")
                    cancelBtn.type = "button"
                    cancelBtn.addEventListener("click", removeGuildDeletor, true)
                    var cancelBtnText = document.createTextNode("Cancel")
                    cancelBtn.appendChild(cancelBtnText)
                footer.appendChild(cancelBtn)
                var submitBtn = document.createElement("button")
                    submitBtn.classList.add("btn", "btn-danger", "opacity-control")
                    submitBtn.type = "submit"
                    var submitBtnText = document.createTextNode("Delete")
                    submitBtn.appendChild(submitBtnText)
                footer.appendChild(submitBtn)
        ...
        form.addEventListener("submit", sendGuildDel, true)

    return layer
}

function buildGuildRenamer() {
    var layer = document.createElement("div")
        layer.classList.add("guildRenamerLayer", "layer")

        var form = document.createElement("form")
            form.classList.add("guildRenamerForm", "layer-form", "bg-body")

            var header = document.createElement("div")
                header.classList.add("GRF-header", "LF-header")
                var div = document.createElement("div")
                    var divText = document.createTextNode("Rename guild")
                    div.appendChild(divText)
                header.appendChild(div)
                var closeBtn = document.createElement("button")
                    closeBtn.classList.add("btn", "opacity-control")
                    closeBtn.type = "button"
                    closeBtn.addEventListener("click", removeGuildRenamer, true)
                    closeBtn.appendChild(getSVG("close"))
                header.appendChild(closeBtn)
            form.appendChild(header)

            var body = document.createElement("div")
                body.classList.add("GRF-body", "LF-body")
                var label = document.createElement("label")
                ...
                var input = document.createElement("input")
                    ...
                    input.type = "text"
                    input.name = "name"
                    input.value = guildName
                    input.maxLength = maxGuildNameLength
                body.appendChild(input)
            form.appendChild(body)

            var footer = document.createElement("div")
                footer.classList.add("GRF-footer", "LF-footer")
                var cancelBtn = document.createElement("button")
                    cancelBtn.classList.add("btn", "opacity-control")
                    cancelBtn.type = "button"
                    cancelBtn.addEventListener("click", removeGuildRenamer, true)
                    var cancelBtnText = document.createTextNode("Cancel")
                    cancelBtn.appendChild(cancelBtnText)
                footer.appendChild(cancelBtn)
                var submitBtn = document.createElement("button")
                    submitBtn.classList.add("btn", "btn-primary")
                    submitBtn.type = "submit"
                    var submitBtnText = document.createTextNode("Rename guild")
                    submitBtn.appendChild(submitBtnText)
                footer.appendChild(submitBtn)
        ...
        form.addEventListener("submit", sendGuildRename, true)
        input.addEventListener("input", function() { if (input.value.trim() == "") { submitBtn.disabled = true } else { submitBtn.disabled = false } }, true)

    return layer
}

function removeChannelCreator() {
    document.querySelectorAll(".channelCreatorLayer").forEach(ele => ele.remove())
}

function removeGuildDeletor() {
    document.querySelectorAll(".guildDeletorLayer").forEach(ele => ele.remove())
}

function removeGuildRenamer() {
    document.querySelectorAll(".guildRenamerLayer").forEach(ele => ele.remove())
}
...
function getTextWidth(text, font) {
    if (!font) { font = window.getComputedStyle(document.body).font }
    // Vytvoření neviditelného prvku pro měření šířky textu
    const measuringElement = document.createElement('span');
    measuringElement.style.font = font;
    measuringElement.style.visibility = 'hidden';
    measuringElement.style.whiteSpace = 'nowrap';
    measuringElement.innerText = text;
    document.body.appendChild(measuringElement);

    // Získání šířky textu
    const width = measuringElement.offsetWidth;

    // Odstranění měřicího prvku z DOM
...
serverSettings.addEventListener("click", serverSettingsMenu, true)
